...
def dump_customer(self, request):
    return {"@type": "Customer", "@id": request.link(self), "name": self.name}
...
def dump_customer_collection(self, request):
    return {
        ...
        "customers": [request.link(customer) for customer in self.customers.values()],
...
@App.json(model=CustomerCollection, request_method="POST", body_model=Customer)
